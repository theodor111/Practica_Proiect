...
function Nightlife() {
    const { rol } = useUserRole(); // Extragem rolul utilizatorului conectat
    ...
    useEffect(() => {
        const fetchEvenimente = async () => {
            try {
                ...
                // Cerem din Firebase doar categoria "Nightlife"
                const q = query(collection(db, "evenimente"), where("categorie", "==", "Nightlife"));
                ...
                for (const document of querySnapshot.docs) {
                    ...
                    if (dataEveniment < azi) {
                        ...
                        console.log(`Evenimentul nightlife expirat "${ev.titlu}" a fost șters.`);
                    } else {
                ...
                console.error("Eroare la citirea evenimentelor nightlife: ", error);
            }
        };
    ...
    const valideaza = () => {
        ...
        else if (!regexData.test(formular.data)) eroriNoi.data = "Formatul corect este ZZ.LL.AAAA (ex: 30.05.2026).";
        if (!formular.ora.trim()) eroriNoi.ora = "Ora este obligatorie.";
        else if (!regexOra.test(formular.ora)) eroriNoi.ora = "Formatul corect este HH:MM (ex: 22:00).";
    ...
    const adaugaEveniment = async (e) => {
        ...
        try {
            const documentNou = {
                ...formular,
                tip_nightlife: formular.categorie, // Salvăm tipul (Club, Karaoke etc.)
                categorie: "Nightlife", // Secțiunea principală
    ...
    const evenimenteFiltrate = (() => {
        let lista = categorieActiva === "Toate" 
            ? [...evenimente] 
            : evenimente.filter((e) => e.tip_nightlife === categorieActiva);
    ...
    return (
        <div className="app-root">
            ...
            <main>
                    ...
                    <section className="categories" id="adauga-nightlife">
                        ...
                        <form className="event-form" onSubmit={adaugaEveniment}>
                            <div className="form-group">
                                ...
                                <input type="text" name="titlu" placeholder="Ex: Party @ Club X" value={formular.titlu} onChange={schimbaCamp} />
                            ...
                            <div className="form-row">
                                <div className="form-group"><label>Data *</label><input type="text" name="data" placeholder="Ex: 30.05.2026" value={formular.data} onChange={schimbaCamp} />{erori.data && <p className="eroare">{erori.data}</p>}</div>
                                <div className="form-group"><label>Ora *</label><input type="text" name="ora" placeholder="Ex: 22:00" value={formular.ora} onChange={schimbaCamp} />{erori.ora && <p className="eroare">{erori.ora}</p>}</div>
                            </div>
                            <div className="form-group"><label>Locația *</label><input type="text" name="locatie" placeholder="Club/Bar" value={formular.locatie} onChange={schimbaCamp} />{erori.locatie && <p className="eroare">{erori.locatie}</p>}</div>
                            <div className="form-group"><label>Cost</label><input type="text" name="cost" placeholder="Bilet / Intrare" value={formular.cost} onChange={schimbaCamp} />{erori.cost && <p className="eroare">{erori.cost}</p>}</div>
                            <div className="form-group"><label>Descriere</label><textarea name="descriere" placeholder="Detalii party" value={formular.descriere} onChange={schimbaCamp} /></div>
                ...
                <section className="categories" id="nightlife-lista">
                    ...
                    {evenimenteFiltrate.length > 0 ? (
                            ...
                            {evenimenteFiltrate.map((ev) => (
                                <div className="card event-card" key={ev.id}>
                                    ...
                                    <div className="card-body event-card-body">
                                        <h3>{ev.titlu}</h3>
                                        <p className="event-category">{ev.tip_nightlife}</p>
                        ...
                        <p style={{textAlign: "center", color: "#aaa", marginTop: "20px"}}>Nu a fost găsit niciun eveniment nightlife.</p>
                    )}
                </section>
            </main>
            ...
            <ChatWidget currentEventId="nightlife" />
        </div>
    );
}
