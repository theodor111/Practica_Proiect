...
const Register = () => {
  const [formData, setFormData] = useState({ name: '', email: '', password: '', confirmPassword: '', rol: 'vizitator' });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // 1. Creează contul în Auth
      const cred = await createUserWithEmailAndPassword(auth, formData.email, formData.password);
      await updateProfile(cred.user, { displayName: formData.name });
      
      // LOGICA DE APROBARE:
      const rolFinal = formData.rol === 'organizator' ? 'in_asteptare' : 'vizitator';

      // 2. Salvează ROLUL în baza de date
      await setDoc(doc(db, 'users', cred.user.uid), {
        name: formData.name,
        email: formData.email,
        rol: rolFinal, // Aici salvăm 'vizitator' sau 'in_asteptare'
        createdAt: new Date()
      });

      // 3. Mesaj de informare pentru viitorii organizatori
      if (rolFinal === 'in_asteptare') {
          alert("Cont creat cu succes! Cererea ta de Organizator este în așteptare. Până la aprobare de către admin, poți explora evenimentele ca vizitator.");
      }

      navigate('/'); // Succes! Mergem pe pagina principală
  ...
  return (
    <div style={{ 
      ...
      padding: "20px", 
      position: "relative",
      backgroundImage: 'linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url("/cont3.jpeg")', 
      ...
      <div style={{ background: CARD_BG, backdropFilter: 'blur(22px)', border: BORDER, borderRadius: '20px', padding: '50px 40px', maxWidth: '450px', width: '100%', boxShadow: `0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(153,15,75,0.1)`, animation: 'fadeEntry 1s ease-out forwards' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '28px' }}>
            <label style={{ fontFamily: 'Oswald, sans-serif', fontSize: '0.8rem', letterSpacing: '1px', textTransform: 'uppercase', color: '#aaa', display: 'block', marginBottom: '12px' }}>
              Tip cont
            </label>
            <div style={{ display: 'flex', gap: '12px' }}>
              {['vizitator', 'organizator'].map((rol) => (
                <button
                  key={rol}
                  type="button"
                  ...
                  style={{
                    ...
                    padding: '12px',
                    borderRadius: '10px',
                    border: formData.rol === rol ? `2px solid #990f4b` : '2px solid rgba(255,255,255,0.15)',
                    background: formData.rol === rol ? 'rgba(153,15,75,0.2)' : 'rgba(255,255,255,0.04)',
                    color: formData.rol === rol ? 'white' : '#aaa',
                    ...
                    cursor: 'pointer',
                    transition: 'all 0.3s ease',
                    boxShadow: formData.rol === rol ? '0 0 14px rgba(153,15,75,0.35)' : 'none',
                  }}
                >
                  {rol === 'vizitator' ? ' Vizitator' : ' Organizator'}
                </button>
              ))}
            </div>
            <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '0.78rem', color: '#666', marginTop: '8px' }}>
              {formData.rol === 'organizator'
                ? 'Cererea ta va fi analizată de un administrator.'
                : 'Vei putea explora și filtra evenimente imediat.'}
            </p>
          </div>
